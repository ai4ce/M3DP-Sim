...
void Emitter::emitParticlesCircle(std::vector <unsigned int> &reusedParticles, unsigned int &indexReuse, unsigned int &numEmittedParticles)
{
	...
	if (t >= m_emitStartTime && t <= m_emitEndTime) //-0.25
	{
		// animate emitted particles		
		const Vector3r & x0 = m_x;
		...
		//const Vector3r & test = Vector3r(0.0,2.0,0.0);
		//const Vector3r & pos = x0 + Vector3r(5.0,5.0,5.0);
		//const Vector3r &pos = Vector3r(0.0,2.0,2.0);
		//std::cout << " emmiter update 2 " << std::endl; 
	...
	const Vector3r offset = m_x + velocityOffset;
	//const Vector3r offset = m_x + Vector3r(0.5*sin(2*t),0.5*cos(2*t),0);
	//const Vector3r offset = Vector3r(xpos,ypos,zpos);	
		...
		for (unsigned int i = 0; i < m_width; i++)
		{
			for (unsigned int j = 0; j < m_width; j++)
			{
				...
				if ((index < m_model->numParticles()) && (x*x + y*y <= radius2))
				{
					...
					//m_model->getPosition(index) = offset;
...
void Emitter::move_emitter(const Vector3r mover)
{
	m_x = mover;
}
